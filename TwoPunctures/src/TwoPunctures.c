...
static derivs tp_v = {0};
static derivs tp_cf_v = {0};
static int tp_n1 = 0;
static int tp_n2 = 0;
static int tp_n3 = 0;
static int tp_solved = 0;
...
static CCTK_REAL
evaluate_psi_at_point(CCTK_REAL xx, CCTK_REAL yy, CCTK_REAL zz)
{
  DECLARE_CCTK_PARAMETERS;

  if (!tp_solved) {
    CCTK_WARN(0, "TwoPunctures metric requested before the solve completed.");
    return NAN;
  }
  ...
  CCTK_REAL U;
  if (CCTK_EQUALS(grid_setup_method, "evaluation")) {
    U = PunctIntPolAtArbitPositionFast(0, 1, tp_n1, tp_n2, tp_n3, tp_cf_v, xx, yy, zz);
  } else {
    U = PunctTaylorExpandAtArbitPosition(0, 1, tp_n1, tp_n2, tp_n3, tp_v, xx, yy, zz);
  }
  ...
  CCTK_REAL psi1 = 1 + 0.5 * par_m_plus / r_plus + 0.5 * par_m_minus / r_minus + U;
...
#define EXTEND(M, r) \
  ...
  if (r_plus < TP_Extend_Radius) {
    psi1 = 1 + 0.5 * EXTEND(par_m_plus, r_plus) + 0.5 * par_m_minus / r_minus + U;
  }
  if (r_minus < TP_Extend_Radius) {
    psi1 = 1 + 0.5 * EXTEND(par_m_minus, r_minus) + 0.5 * par_m_plus / r_plus + U;
  }

  CCTK_REAL static_psi = 1.0;
  ...
  return pow(psi1 / static_psi, 4);
}

CCTK_REAL
TwoPunctures_EvaluatePsiAtPoint(CCTK_POINTER_TO_CONST cctkGH,
                                CCTK_REAL x,
                                CCTK_REAL y,
                                CCTK_REAL z)
{
  DECLARE_CCTK_PARAMETERS;
  (void) cctkGH;
  return evaluate_psi_at_point(x - center_offset[0],
                                      y - center_offset[1],
                                      z - center_offset[2]);
}
