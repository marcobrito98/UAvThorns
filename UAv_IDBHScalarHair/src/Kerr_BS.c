  ...
  for (int k = 0; k < cctk_lsh[2]; ++k) {
    for (int j = 0; j < cctk_lsh[1]; ++j) {
      for (int i = 0; i < cctk_lsh[0]; ++i) {
        ...
        const CCTK_REAL dbetadphi_dR = dr_dR*2*bh_spin*bh_mass*(rBL*rBL-bh_spin2*costh2)*sinth2/pow(rho2kerr,2);//aqui nao é rho2kerr no numerador. corrigido.
        ...
        const CCTK_REAL dgammaphiphi_dR= dr_dR*2*(rr_2*(rBL+bh_spin2*(bh_mass-rBL)*sinth2))/(rr2_2*rr_2) + \
                                        2*(-bh_spin2-rBL*rBL+bh_spin2*delta_metric*sinth2)/(rr2_2*rr_2);//será que as derivadas estão bem??
//faltava um 2 acima no segundo termo e tambem um r quadrado na linha de cima
