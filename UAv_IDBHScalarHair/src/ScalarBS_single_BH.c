...
void UAv_IDScalarBS_single(CCTK_ARGUMENTS) {
  ...
  for (int k = 0; k < cctk_lsh[2]; ++k) {
    for (int j = 0; j < cctk_lsh[1]; ++j) {
      for (int i = 0; i < cctk_lsh[0]; ++i) {
        ...
        //   fprintf(stderr,"x1 = %lf, y1 = %lf, z1 = %lf\n", x1, y1, z1);
        //   abort();
        ...
        // 3-metric
        gxx[ind] = gxx[ind] + conf_fac * (1. + h_rho2 * sinph * sinph) - 1;
        gxy[ind] = gxy[ind] + (-conf_fac * h_rho2 * sinph * cosph);
        gxz[ind] = gxz[ind] + 0;
        gyy[ind] = gyy[ind] + conf_fac * (1. + h_rho2 * cosph * cosph) - 1;
        gyz[ind] = gyz[ind] + 0;
        gzz[ind] = gzz[ind] + conf_fac - 1;
        ...
        */
        ...
        // extrinsic curvature
        kxx[ind] = kxx[ind] + 0.5 * rho * sin(2 * ph) * exp_auxi * dW_drho;
        kxy[ind] = kxy[ind] + (-0.5 * rho * cos(2 * ph) * exp_auxi * dW_drho);
        kxz[ind] = kxz[ind] + 0.5 * y1 * exp_auxi * dW_dz;
        kyy[ind] = kyy[ind] + 0.5 * rho * sin(2 * ph) * exp_auxi * dW_drho;
        kyz[ind] = kyz[ind] + (-0.5 * x1 * exp_auxi * dW_dz);
        kzz[ind] = kzz[ind] + 0.;
