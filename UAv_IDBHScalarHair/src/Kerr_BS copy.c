  ...
  for (int k = 0; k < cctk_lsh[2]; ++k) {
    for (int j = 0; j < cctk_lsh[1]; ++j) {
      for (int i = 0; i < cctk_lsh[0]; ++i) {
        ...
        const CCTK_REAL dalpha_dx = (2 * pow(rr_2, 2) * (pow(rBL, 2) + pow(bh_spin, 2) * (1 - (-1 + costh2) * sigma)) * R_x +
                                     2 * pow(horizon_radius, 2) * (pow(rBL, 2) + pow(bh_spin, 2) * (1 - (-1 + costh2) * sigma)) * R_x +
                                     rr_2 * pow(horizon_radius, 2) * (2 * rBL * R_x * dr_dR + pow(bh_spin, 2) * (-(sigma * costh2_x) - (-1 + costh2) * dsigma_dx)) +
                                     pow(rr_2, 3) * (-2 * rBL * R_x * dr_dR + pow(bh_spin, 2) * (sigma * costh2_x + (-1 + costh2) * dsigma_dx))) /
                                    (2. * pow(rr_2, 2) * pow(pow(rBL, 2) + pow(bh_spin, 2) * (1 - (-1 + costh2) * sigma), 1.5));
        const CCTK_REAL dalpha_dy = (2 * pow(rr_2, 2) * (pow(rBL, 2) + pow(bh_spin, 2) * (1 - (-1 + costh2) * sigma)) * R_y + 2 * pow(horizon_radius, 2) * (pow(rBL, 2) + pow(bh_spin, 2) * (1 - (-1 + costh2) * sigma)) * R_y +
                                     rr_2 * pow(horizon_radius, 2) * (2 * rBL * R_y * dr_dR + pow(bh_spin, 2) * (-(sigma * costh2_y) - (-1 + costh2) * dsigma_dy)) +
                                     pow(rr_2, 3) * (-2 * rBL * R_y * dr_dR + pow(bh_spin, 2) * (sigma * costh2_y + (-1 + costh2) * dsigma_dy))) /
                                    (2. * pow(rr_2, 2) * pow(pow(rBL, 2) + pow(bh_spin, 2) * (1 - (-1 + costh2) * sigma), 1.5));
        const CCTK_REAL dalpha_dz = (2 * pow(rr_2, 2) * (pow(rBL, 2) + pow(bh_spin, 2) * (1 - (-1 + costh2) * sigma)) * R_z + 2 * pow(horizon_radius, 2) * (pow(rBL, 2) + pow(bh_spin, 2) * (1 - (-1 + costh2) * sigma)) * R_z +
                                     rr_2 * pow(horizon_radius, 2) * (2 * rBL * R_z * dr_dR + pow(bh_spin, 2) * (-(sigma * costh2_z) - (-1 + costh2) * dsigma_dz)) +
                                     pow(rr_2, 3) * (-2 * rBL * R_z * dr_dR + pow(bh_spin, 2) * (sigma * costh2_z + (-1 + costh2) * dsigma_dz))) /
                                    (2. * pow(rr_2, 2) * pow(pow(rBL, 2) + pow(bh_spin, 2) * (1 - (-1 + costh2) * sigma), 1.5));
