  ...
  for (int k = 0; k < cctk_lsh[2]; ++k) {
    for (int j = 0; j < cctk_lsh[1]; ++j) {
      for (int i = 0; i < cctk_lsh[0]; ++i) {
        ...
        h1xx = conf_fac * (1. + h_rho2 * sinph * sinph); //não há argumentos? E onde e que defino as variaveis?
        h1xy = -conf_fac * h_rho2 * sinph * cosph;
        h1xz = 0;
        h1yy = conf_fac * (1. + h_rho2 * cosph * cosph);
        h1yz = 0;
        h1zz = conf_fac;

        h2xx = conf_fac * (1. + h_rho2 * sinph_2 * sinph_2);
        h2xy = -conf_fac * h_rho2 * sinph_2 * cosph_2;
        h2xz = 0;
        h2yy = conf_fac * (1. + h_rho2 * cosph_2 * cosph_2);
        h2yz = 0;
        h2zz = conf_fac;
        ...
        */
        ...
        k1xx[ind] =  0.5 * rho * sin(2*ph) * exp_auxi * dW_drho;
        k1xy[ind] = -0.5 * rho * cos(2*ph) * exp_auxi * dW_drho;
        k1xz[ind] =  0.5 *  y1 * exp_auxi * dW_dz;
        k1yy[ind] = -k1xx[ind];
        k1yz[ind] = -0.5 *  x1 * exp_auxi * dW_dz;
        k1zz[ind] =  0.;

        k2xx[ind] =  0.5 * rho_2 * sin(2*ph_2) * exp_auxi * dW_drho;
        k2xy[ind] = -0.5 * rho_2 * cos(2*ph_2) * exp_auxi * dW_drho;
        k2xz[ind] =  0.5 *  y2 * exp_auxi * dW_dz;
        k2yy[ind] = -k2xx[ind];
