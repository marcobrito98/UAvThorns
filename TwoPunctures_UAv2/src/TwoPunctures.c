/* UAV2_TwoPunctures:  File  "UAV2_TwoPunctures.c"*/
...
/* -------------------------------------------------------------------*/
void UAV2_TwoPunctures(CCTK_ARGUMENTS);
void
UAV2_TwoPunctures (CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS_UAV2_TwoPunctures;
  DECLARE_CCTK_PARAMETERS;
    ...
    if(!(give_bare_mass)) {
      ...
      do {
        ...
        *mm = (tmp - M_p + M_m)/(2.*(1 + um));
        ...
        CCTK_ParameterSet ("par_m_plus", "TwoPunctures_UAv2", valbuf);
        
        sprintf (valbuf,"%.17g", (double) *mm);
        CCTK_ParameterSet ("par_m_minus", "TwoPunctures_UAv2", valbuf);
...
#define EXTEND(M,r) \
        ...
        if ((*conformal_state > 0) || (pmn_lapse) || (brownsville_lapse)) {
          ...
          if (pmn_lapse)
            alp_ref[ind] = pow(p, initial_lapse_psi_exponent);
          if (brownsville_lapse)
            alp_ref[ind] = 2.0/(1.0+pow(p, initial_lapse_psi_exponent));

        } /* if conformal-state > 0 */
          
        puncture_u[ind] = U;

        gxx_ref[ind] = pow (psi1 / static_psi, 4);
        gxy_ref[ind] = 0;
        gxz_ref[ind] = 0;
        gyy_ref[ind] = pow (psi1 / static_psi, 4);
        gyz_ref[ind] = 0;
        gzz_ref[ind] = pow (psi1 / static_psi, 4);

        kxx_ref[ind] = Aij[0][0] / pow(psi1, 2);
        kxy_ref[ind] = Aij[0][1] / pow(psi1, 2);
        kxz_ref[ind] = Aij[0][2] / pow(psi1, 2);
        kyy_ref[ind] = Aij[1][1] / pow(psi1, 2);
        kyz_ref[ind] = Aij[1][2] / pow(psi1, 2);
        kzz_ref[ind] = Aij[2][2] / pow(psi1, 2);
        ...
        if (antisymmetric_lapse || averaged_lapse) {
          ...
          if (averaged_lapse) {
            alp_ref[ind] = 0.5 * (1.0 + alp_ref[ind]);
          }
        }
        if (multiply_old_lapse)
          alp_ref[ind] *= old_alp;
