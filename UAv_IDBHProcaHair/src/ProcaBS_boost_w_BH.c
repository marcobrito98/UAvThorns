    ...
    for (int j = 0; j < cctk_lsh[1]; ++j) {
      for (int i = 0; i < cctk_lsh[0]; ++i) {
        ...
        */
        ...
        // The closed-form inverse above assumes the unrotated axisymmetric
        // metric. Use the actual rotated/boosted spatial metric instead.
        CCTK_REAL gammaA_metric[4][4] = {{0.}};
        gammaA_metric[1][1] = Gb[1][1];
        gammaA_metric[1][2] = Gb[1][2];
        gammaA_metric[1][3] = Gb[1][3];
        gammaA_metric[2][1] = Gb[2][1];
        gammaA_metric[2][2] = Gb[2][2];
        gammaA_metric[2][3] = Gb[2][3];
        gammaA_metric[3][1] = Gb[3][1];
        gammaA_metric[3][2] = Gb[3][2];
        gammaA_metric[3][3] = Gb[3][3];
        if (!invert_spd3x3(gammaA_metric, gammaA_inv)) {
          CCTK_VERROR("Could not invert the rotated/boosted spatial metric.");
        }
